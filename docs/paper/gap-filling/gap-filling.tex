...
\begin{document}
	...
	\begin{abstract}
		Digital Terrain Models (DTMs) generated from HiRISE imagery are essential for Martian exploration but frequently suffer from data gaps due to stereo-matching failures in low-texture or shadowed regions. This work proposes a Deep Learning approach using Vision Transformers (ViT), specifically the Dense Prediction Transformer (DPT) architecture, to infer missing topography from monocular orthophotos. We curated a dataset of valid DTM-Orthophoto pairs to train the model in a supervised manner. Results demonstrate that the ViT-based model successfully recovers morphological structures, achieving a Structural Similarity Index (SSIM) of 0.99 and a Root Mean Square Error (RMSE) of 3.33 meters in plain terrains. The proposed pipeline offers a robust, automated solution to generate analysis-ready products for planetary science.
	\end{abstract}
	
	\begin{resumo}
		Modelos Digitais de Terreno (DTMs) gerados a partir de imagens HiRISE são essenciais para a exploração marciana, mas frequentemente apresentam lacunas de dados devido a falhas na correspondência estéreo em regiões de baixa textura ou sombra. Este trabalho propõe uma abordagem de Aprendizado Profundo utilizando Vision Transformers (ViT), especificamente a arquitetura Dense Prediction Transformer (DPT), para inferir a topografia ausente a partir de ortofotos monoculares. Curou-se um conjunto de dados de pares DTM-Ortoimagem válidos para treinamento supervisionado. Os resultados demonstram que o modelo baseado em ViT recupera com sucesso estruturas morfológicas, atingindo um Índice de Similaridade Estrutural (SSIM) de 0,99 e um erro médio quadrático (RMSE) de 3,33 metros em terrenos planos. O pipeline proposto oferece uma solução robusta e automatizada para gerar produtos prontos para análise na ciência planetária.
	\end{resumo}
