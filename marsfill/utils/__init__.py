...
class ApplicationLogger:
    ...
    def __init__(self, log_file_path: Optional[Path] = None) -> None:
        ...
        try:
            ...
            print(
                f"Aviso: Não foi possível criar arquivo de log em {log_file_path}. Apenas logs de console estarão ativos."
            )
...
def find_digital_terrain_model_candidate(
    candidate_files_list: List[CandidateFile],
...
def find_candidate_by_type(
    candidate_files_list: List[CandidateFile], target_type: CandidateType
) -> Optional[CandidateFile]:
    """
...
def load_dataset_files(
    base_directory: Path, orthophoto_pattern: str, digital_terrain_model_pattern: str
    ...
    if not found_dtm_files:
        logger.error(
            f"Nenhum arquivo '{digital_terrain_model_pattern}' encontrado em {base_directory}"
        )
...
def validate_dataset_pairs(
    ortho_files_list: List[str], dtm_files_list: List[str]
) -> Tuple[List[str], List[str]]:
    """
    ...
    for orthophoto_path, dtm_path in zip(ortho_files_list, dtm_files_list):
        if not os.path.exists(dtm_path):
            logger.error(
                f"Arquivo DTM correspondente {dtm_path} não encontrado para {orthophoto_path}"
            )
